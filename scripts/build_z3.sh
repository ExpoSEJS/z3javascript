...
sed -i 's/^\(CXXFLAGS=.*\)/\1 -s EXPORTED_FUNCTIONS="[$(cat ./compiled/z3_bindings_flat)]"/g' config.mk
sed -i 's/^\(LINK_EXTRA_FLAGS=.*\)/\1 -O3 -fPIC \-s EXPORTED_FUNCTIONS="[$(cat ./compiled/z3_bindings_flat)]"/g' config.mk
emmake make
